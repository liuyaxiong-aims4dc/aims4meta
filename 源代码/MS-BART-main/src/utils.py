...
# Refer: https://github.com/pluskal-lab/MassSpecGym/blob/main/massspecgym/utils.py

def get_canonical_smiles(selfies):
    try:
        smiles1 = decoder(selfies)
        mol1 = Chem.MolFromSmiles(smiles1)
        canonical_smiles1 = Chem.MolToSmiles(mol1, canonical=True)
        return canonical_smiles1
    except Exception:
        return None
...
def compare_selfies(selfies1, selfies2):
    try:
        smiles1 = decoder(selfies1)
        mol1 = Chem.MolFromSmiles(smiles1)
        canonical_smiles1 = Chem.MolToSmiles(mol1, canonical=True)
        # inchis1 = Chem.MolToInchi(mol1)

        smiles2 = decoder(selfies2)
        mol2 = Chem.MolFromSmiles(smiles2)
        canonical_smiles2 = Chem.MolToSmiles(mol2, canonical=True)
        # inchis2 = Chem.MolToInchi(mol2)

        if canonical_smiles1 == canonical_smiles2: return True
        else: return False
        # if inchis1 == inchis2: return True
        # else: return False
    except Exception:
        return False
...
def selfies_to_formula(selfies):
    ...
    hill_order = ['C', 'H'] if 'C' in formula else []
    other_elements = sorted([e for e in formula.keys() if e not in hill_order])
    ordered_elements = hill_order + other_elements
    formula_str = ""
    for elem in ordered_elements:
        for count in [formula[elem]]:
            if count > 1:
...
def print_trainable_parameters(model):
    ...
    print(
        ...
        f"Non-trainable parameters: {all_params - trainable_params:,}\n"
