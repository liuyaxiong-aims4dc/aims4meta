...
if __name__ == "__main__":
    dataset_name = "msg"
    df = pd.read_csv(f"./{dataset_name}/{dataset_name}-demo.tsv", sep="\t")
    meta_spec_list: List[Tuple[dict, List[Tuple[str, np.ndarray]]]] = []
    label_entries = []
    ...
    for _, row in df.iterrows():
        ...
        meta = {
            ...
            "parentmass": float(row["parent_mass"]),
        }
    ...
    with open(f"./{dataset_name}/{dataset_name}-demo.mgf", "w") as f:
        f.write(mgf_output)

    # ==================== Generate label.tsv ====================
    label_df = pd.DataFrame(label_entries)
    print("After processing, the number of entries is: ", len(label_entries))
    label_df = label_df[["spec", "formula", "ionization", "dataset", "compound", "parentmass", "instrument"]]  # Ensure the column order is correct.
    label_df.to_csv(f"./{dataset_name}/{dataset_name}-labels.tsv", sep="\t", index=False)
