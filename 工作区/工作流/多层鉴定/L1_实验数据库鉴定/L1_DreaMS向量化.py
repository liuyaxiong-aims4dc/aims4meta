...
# 模式判断：
# - 单文件模式：L1_MSP_FILE 指定单个 MSP/mzML 文件路径，或 JSON 列表（多个文件自动合并）
# - 数据库模式：L1_DATABASES 指定数据库配置 JSON

MSP_FILE = os.environ.get('L1_MSP_FILE')  # 单文件模式（支持JSON列表，支持MSP/mzML格式）
...
def compute_embedding_mzml(mzml_path, output_npz, batch_size=64):
    """直接处理mzML文件计算DreaMS embedding（无元数据）"""
    print(f"  加载 DreaMS 模型...", flush=True)
    from dreams.api import dreams_embeddings

    print(f"  计算DreaMS embedding (mzML)...", flush=True)
    embeddings = dreams_embeddings(mzml_path, batch_size=batch_size, progress_bar=True)

    if embeddings.ndim == 1:
        embeddings = embeddings.reshape(1, -1)

    # 保存embedding（mzML无元数据）
    save_dict = {'embeddings': embeddings.astype(np.float32)}
    
    np.savez_compressed(output_npz, **save_dict)
    print(f"  保存: {output_npz} (shape={embeddings.shape}, 无元数据)")
    
    # 清理DreaMS生成的HDF5缓存
    hdf5_path = mzml_path.replace('.mzML', '.hdf5').replace('.mzml', '.hdf5')
...
def process_database(db_name, msp_path, force=False, batch_size=64, output_dir=None):
    ...
    # 判断文件格式
    is_mzml = msp_path.suffix.lower() == '.mzml'
    ...
    if os.path.exists(output_npz) and not force:
        ...
        # 检查PKL缓存是否存在，不存在则单独生成（仅MSP格式）
        if not is_mzml and not os.path.exists(pkl_path):
    ...
    # mzML格式：直接使用DreaMS处理
    if is_mzml:
        print(f"  [INFO] 检测到mzML格式，使用DreaMS原生处理...")
        shape = compute_embedding_mzml(str(msp_path), output_npz, batch_size)
        return True, output_npz
    
    # MSP格式：MSP → 临时MGF + 元数据 + 谱图缓存
...
def main():
    ...
    if MSP_FILE:
        ...
        if msp_files and len(msp_files) > 1:
            ...
            # 从文件名推断离子模式（POS/NEG）
            ion_mode = "UNKNOWN"
            first_filename = Path(msp_files[0]).name.upper()
            if "_POS." in first_filename or "_POS_" in first_filename:
                ion_mode = "POS"
            elif "_NEG." in first_filename or "_NEG_" in first_filename:
                ion_mode = "NEG"

            merged_msp = os.path.join(first_dir, f"simulated_merged_{ion_mode}.msp")

            # 清除旧的合并缓存（MSP内容变化时，旧缓存会导致加载失败）
            for ext in ['_matchms_cache.pkl', '_spectra_cache.pkl', '_dreams_emb.npz']:
                old_cache = merged_msp.replace('.msp', ext)
