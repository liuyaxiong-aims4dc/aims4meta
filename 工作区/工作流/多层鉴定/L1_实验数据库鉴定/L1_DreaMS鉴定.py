#!/usr/bin/env python3
"""
L1: DreaMS Embedding鉴定（方法导向）

鉴定策略：DreaMS embedding cosine相似度方法遍历所有数据库
...
# 碎片鉴定（优先使用Numba加速）
_numba_frag_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    '..', '辅助功能', 'Numba碎片匹配', 'numba_fragment_matching.py'
)
if os.path.exists(_numba_frag_path):
    import importlib.util as _ilu
    _spec_nb = _ilu.spec_from_file_location(
        "numba_fragment_matching", os.path.abspath(_numba_frag_path))
    _numba_frag_module = _ilu.module_from_spec(_spec_nb)
...
def match_with_dreams_fast(query_emb, candidates, lib_embs_norm, lib_metadata,
    ...
    """
    快速DreaMS embedding鉴定（无需谱图，仅用元数据）
    
    返回: (matches_list, stats) - matches_list 是按分数排序的鉴定列表，最多 top_k 个
    """
    if not candidates:
...
def match_with_dreams(query_spectrum, query_emb, candidates, lib_embs_norm,
    ...
    # 过滤母离子峰，避免虚高碎片计数
    # 用 3× tolerance 窗口：MSP metadata 的 precursor_mz 可能与实际峰 m/z 偏差 0.01–0.03 Da
    query_pmz = query_spectrum.get("precursor_mz")
    precursor_window = fragment_tolerance * 3
    if query_pmz:
        query_peaks = [(mz, inten) for mz, inten in query_peaks
                       if abs(mz - query_pmz) > precursor_window]
    ...
    for local_idx in sorted_idx:
        ...
        try:
            ...
            lib_peaks_count = len(lib_norm.peaks.mz)
            ...
            real_matched_peaks = len(matched_frags)

            if real_matched_peaks >= min_matched_peaks:
                passed_fragments += 1
                all_matches.append({
                    "library_index": lib_idx,
                    "cosine_score": emb_score,
                    "matched_peaks": real_matched_peaks,
                    "lib_peaks_count": lib_peaks_count,
...
def main():
    """主函数 - DreaMS Embedding鉴定，遍历所有数据库"""
    ...
    # 命令行参数（可覆盖环境变量）
    ###########################################################################
    parser = argparse.ArgumentParser(description="L1 DreaMS Embedding鉴定（方法导向，多库遍历）")
    ...
    parser.add_argument("--min_matched_peaks", type=int, default=MIN_MATCHED_PEAKS, help="最少鉴定碎片数")
    ...
    print("=" * 70)
    print(f"{level} DreaMS Embedding鉴定（方法导向，多库遍历）")
    print("=" * 70)
    print(f"[参数] 母离子容差: {args.mz_ppm} ppm")
    print(f"[参数] 碎片容差: {args.fragment_tolerance} Da")
    print(f"[参数] 最少鉴定碎片: {args.min_matched_peaks}")
    ...
    if len(sample_embs) != len(sample_spectra):
        print(f"[WARN] embedding数量({len(sample_embs)})与谱图数量({len(sample_spectra)})不一致，仍将继续鉴定")
    ...
    for lib_name, lib_config in LIBRARIES.items():
        msp_path = lib_config["msp"]
        emb_path = lib_config["emb"]
        ...
        if has_meta:
            ...
            if msp_path and os.path.exists(msp_path):
                print(f"[INFO] {lib_name} 加载MSP谱图用于碎片鉴定...")
                libraries[lib_name] = load_library(lib_name, msp_path)
            else:
                print(f"[WARN] {lib_name} MSP文件不存在，将无法进行碎片鉴定")
    ...
    if not all_lib_names:
        print("[WARN] 没有可用的数据库，输出空结果")
        # 输出空结果文件
        output_csv = args.output_csv or os.path.join(args.output_dir, "L1_DreaMS_results.csv")
        output_df = pd.DataFrame(columns=[
            'sample_id', 'sample_precursor_mz', 'sample_rt',
            'library_name', 'compound_name', 'inchikey', 'smiles',
            'library_precursor_mz', 'cosine_score', 'matched_peaks',
            'embedding_similarity', 'rank'
        ])
        output_df.to_csv(output_csv, index=False)
        print(f"[INFO] 空结果已保存: {output_csv}")
        return 0
    
    print(f"[INFO] 开始鉴定 {len(sample_spectra)} 个样品谱图...")
    
    mz_tol = args.mz_ppm / 1e6
    
    for i, query_spectrum in enumerate(tqdm(sample_spectra, desc="[DreaMS鉴定]", unit="条")):
        query_pmz = query_spectrum.get("precursor_mz")
        if not query_pmz:
            continue
        ...
        if top_matches:
            ...
            for rank, match in enumerate(top_matches, 1):
                ...
                if "lib_idx" in match:
                    ...
                    lib_adduct = lib_spectrum.get("adduct") or lib_spectrum.get("precursor_type") or lib_spectrum.get("PRECURSORTYPE") or ""
                ...
                results.append({
                    ...
                    "matched_peaks_ratio": f"\t{match.get('matched_peaks', 0)}/{match.get('lib_peaks_count', 0)}",
    ...
    print("\n" + "=" * 70)
    print(f"{level} DreaMS Embedding鉴定完成")
    print("=" * 70)
    print(f"总样品数:       {len(sample_spectra)}")
    print(f"鉴定成功:       {len(matched_samples)} ({len(matched_samples)/len(sample_spectra)*100:.1f}%)")
