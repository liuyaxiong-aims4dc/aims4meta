...
# 可填: "BIO", "PUBCHEM", "ALL" 等，支持多个；空列表=关闭在线库
# 完整列表参考: https://boecker-lab.github.io/sirius-cli/struct/identifications.html
L3_ONLINE_DATABASES = ["BIO"]  # 例: ["BIO"] 或 ["BIO", "PUBCHEM"]
...
def main():
    ...
    if not args.skip_l3:
        ...
        # 根据开关构建L3数据库列表：本地库 + 在线库
        l3_databases = list(L3_ONLINE_DATABASES)  # 在线库（空列表=纯离线）
        l3_local_db_names = []  # 本地库名列表（用于--databases）

        # 本地数据库开关映射
        l3_db_switches = {
            'tcmbank_herb': L3_USE_TCM,
            'coconut_0425': L3_USE_COCONUT,
            'drugbank': L3_USE_DRUGBANK,
            'fda_approved': L3_USE_FDA,
            'pfas': L3_USE_PFAS,
            'mycotoxin': L3_USE_MYCOTOXIN,
            'public_spectra_2506': L3_USE_PUBLIC_SPECTRA,
        }
        for db_name, enabled in l3_db_switches.items():
            if _on(enabled):
                db_path = L3_LOCAL_DATABASES.get(db_name)
                if db_path and os.path.exists(db_path):
                    l3_local_db_names.append(db_name)
                else:
                    logger.warning(f"[L3] 本地数据库 {db_name} 已启用但文件不存在: {db_path}")

        # 合并本地库与在线库传给 SIRIUS
        # 注意：SIRIUS 对 --databases 是并集查询（合并所有库候选后统一打分），参数顺序不代表优先级/兜底关系
        all_databases = l3_local_db_names + l3_databases

        if not all_databases:
            logger.warning("[L3] 未启用任何数据库（本地库和在线库均为空），SIRIUS结构搜索将无数据库可用")
            logger.warning("[L3] 请在总控脚本中启用至少一个L3本地库开关，或在 L3_ONLINE_DATABASES 中填写在线库名")
