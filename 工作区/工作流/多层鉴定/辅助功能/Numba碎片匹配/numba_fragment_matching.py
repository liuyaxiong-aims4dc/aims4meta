...
使用Numba JIT编译加速碎片鉴定,预期性能提升10-100倍
"""
...
def find_matched_fragments_numba(
    ...
    precursor_window = tolerance * 3
    if query_pmz:
        query_peaks = [(mz, inten) for mz, inten in query_peaks
                       if abs(mz - query_pmz) > precursor_window]
        if not query_peaks:
            return []
    # 库端母离子也必须过滤（MSDIAL MSP 母离子峰常在最前面）
    if lib_pmz:
        lib_peaks = [(mz, inten) for mz, inten in lib_peaks
                     if abs(mz - lib_pmz) > precursor_window]
        if not lib_peaks:
            return []
    ...
    # 转换为旧格式（确保与CPU版round一致：先转Python float再round）
    matched = []
    for i, j, diff in zip(query_idx, lib_idx, mz_diff):
        matched.append({
            "query_mz": round(float(query_mz[i]), 4),
            "lib_mz": round(float(lib_mz[j]), 4),
            "query_intensity": round(float(query_int[i]), 4),
            "lib_intensity": round(float(lib_int[j]), 4),
            "mz_diff": round(float(diff), 4),
        })
...
def find_matched_fragments(
    ...
    if use_numba and NUMBA_AVAILABLE:
        try:
            ...
            print(f"[WARNING] Numba鉴定失败,回退到CPU: {e}")
...
if __name__ == "__main__":
    ...
    for n_peaks in [50, 100, 200, 500]:
        ...
        print(f"  CPU:  {cpu_time*1000:.2f} ms, {len(cpu_results)} 个鉴定")
        print(f"  Numba: {numba_time*1000:.2f} ms, {len(numba_results)} 个鉴定")
        ...
        if len(cpu_results) == len(numba_results):
            ...
            print(f"  ⚠ 鉴定数量不一致: CPU {len(cpu_results)} vs Numba {len(numba_results)}")
