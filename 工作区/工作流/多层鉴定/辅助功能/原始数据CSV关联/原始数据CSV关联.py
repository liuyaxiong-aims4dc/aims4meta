...
2. 提取同位素分布信息并输出为JSON（新增功能，供L5使用）
"""
...
def link_csv_data(input_csv: str, sample_csv: str, output_csv: str, pattern: str) -> bool:
    ...
    # 支持多种列名：query_name（L1-L2/L4标准）, id/featureId（L3 SIRIUS）
    ...
    # 过滤掉原始丰度列（带.1后缀的重复样品列）
    # Progenesis QI导出CSV包含两组数据：Normalised abundance和Raw abundance
    # pandas读取时会为重复列名添加.1后缀，我们只保留归一化丰度（不带后缀的列）
    cols_to_keep = [col for col in sample_df.columns if not col.endswith('.1')]
    sample_df_filtered = sample_df[cols_to_keep]

    # 通过 compound_id 精确鉴定（MSP与CSV一一对应，精确鉴定即可100%关联）
    ...
    print(f"[INFO] Compound ID 精确鉴定: {matched_count}/{len(result_df)} 条")
